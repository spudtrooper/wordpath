class Game {
  ...
  render() {
    ...
    this._letters.render(this, $('#letters').empty());
    if (this.hasSolution()) {
...
class Letters {
  ...
  render(g, el) {
    this._letters.forEach(letter => {
      let l = $('<div>').addClass('letter').addClass('has-letter');
      let ll = $('<div>').text(letter);
      l.append(ll);
      el.append(l);
      $(l).click(() => {
        g.processLetterSelect(letter);
      });
    });
  }
